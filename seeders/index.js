...
async function seedUser () {
  try {
    ...
    userToBeInserted = await dbService.findOne(model.user,{ 'username':'Darian_Padberg50' });
    if (!userToBeInserted) {  
      userToBeInserted = {
        'password':'GpawcrqrE_nLgOD',
        'isDeleted':false,
        'username':'Darian_Padberg50',
        'email':'Karen_Homenick35@hotmail.com',
        'isActive':true,
      ...
      await dbService.update(model.user, { 'username':'Darian_Padberg50' }, userToBeInserted);
    }
    userToBeInserted = await dbService.findOne(model.user,{ 'username':'Kathryn_Veum0' });
    if (!userToBeInserted) {  
      userToBeInserted = {
        'password':'k1zqTcJGu3h3i3z',
        'isDeleted':false,
        'username':'Kathryn_Veum0',
        'email':'Justus.Rowe@gmail.com',
        'isActive':true,
      ...
      userToBeInserted.password = await bcrypt.hash(userToBeInserted.password, 8);
      await dbService.update(model.user, { 'username':'Kathryn_Veum0' }, userToBeInserted);
    }
